...
#  queries = db_session.query(CoinInfo).filter_by(id='11').first() 
  entries = [dict(id=q.id, name=q.name, image_name=q.image_name, code=q.code, place=q.place, amount=q.amount, price_init=q.price_init, memo=q.memo) for q in queries]
  db_session.close()
  return entries
